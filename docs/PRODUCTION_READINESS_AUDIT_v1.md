...
| C3 | Data | **Supabase unavailable:** Many routes return **503** (`community/posts`, `me/favorites`, `sponsor-applications`, etc.). Production requires **reliable DB + migrations** including optional tables (e.g. `torp_platform_notifications`). | `web/src/app/api/**/route.js` grep `503` |
