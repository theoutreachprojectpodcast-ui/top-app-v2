-- ============================================================
-- ONE PASTE: member connections grant + migrate (Production)
-- Safe / idempotent. Paste into Supabase SQL Editor → Run.
-- Project: https://supabase.com/dashboard/project/xbtfoundwmhrqrbcuqcw/sql/new
-- ============================================================

-- 1) Ensure table + blocked_by column + grants
...
-- 2) Backfill from community_follows (no-op if table empty/missing)
do $$
begin
  ...
  insert into public.member_connections (
    requester_profile_id, recipient_profile_id, status, created_at, updated_at
  )
  select
    f.follower_id::uuid,
  ...
  insert into public.member_connections (
    requester_profile_id, recipient_profile_id, status, created_at, updated_at, responded_at
  )
  select
  ...
  insert into public.member_connections (
    requester_profile_id, recipient_profile_id, status, blocked_by_profile_id,
...
-- 3) Quick verify
select
  (select count(*) from public.member_connections) as connection_rows,
  has_table_privilege('service_role', 'public.member_connections', 'select') as service_role_can_select;
