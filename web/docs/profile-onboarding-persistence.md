...
- On first authenticated callback, a profile row is created (profile shell): `top_profiles` in production, `top_qa_profiles` on QA (`qa-the-outreach-project.vercel.app` and Preview). Run `web/supabase/top_qa_profiles_v07_onboarding_parity.sql` on the QA Supabase project if profile save returns column errors.
