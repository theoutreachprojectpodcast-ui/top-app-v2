...
- **WorkOS vs DB fields**: `mergeAccountEmailIntoProfileDto` now fills empty `email`, `firstName`, `lastName`, and `displayName` (derived from IdP names) from `GET /api/me` `user`, so the client profile state matches the signed-in session before every field exists in `top_profiles`.
