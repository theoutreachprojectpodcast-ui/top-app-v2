...
export default function ColorSchemeToggle() {
  ...
  const { isAuthenticated, loadingProfile, profile, setProfile, persistProfile } = useProfileData();
  const isLight = colorScheme === "light";
  async function onToggle() {
    const next = isLight ? "dark" : "light";
    setColorScheme(next);
    if (!isAuthenticated || loadingProfile) return;
    const saved = String(profile?.colorScheme || "").trim().toLowerCase();
    if (saved === next) return;
    setProfile((p) => ({ ...p, colorScheme: next }));
    await persistProfile({ ...profile, colorScheme: next });
  }
