...
/**
 * Web browser gate: guests on protected routes → sign-in; signed-in without membership → paywall.
...
export default function WebAppAccessGate() {
  ...
  useEffect(() => {
    ...
    const hasAccess =
      ...
      hasMobileAppAccess(profile, opts);

    if (!signedIn) {
      if (requiresActiveMembershipPath(path)) {
