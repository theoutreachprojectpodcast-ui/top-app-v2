...
/**
 * Directory search — requires active Pro Membership (or staff entitlement).
 */
export async function POST(request) {
  ...
  if (!admin) {
    return Response.json(
      { ok: false, error: "missing_supabase", message: "Directory is temporarily unavailable." },
      { status: 500 },
    );
  }
  ...
  const supabase = createSupabaseReadClient() || admin;
