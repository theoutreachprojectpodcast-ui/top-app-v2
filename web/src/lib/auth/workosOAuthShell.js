...
export const TORP_OAUTH_SHELL_COOKIE = "torp-oauth-shell";
export const TORP_OAUTH_SHELL_NATIVE = "native";
...
export async function markOAuthNativeShell() {
  ...
  jar.set(TORP_OAUTH_SHELL_COOKIE, TORP_OAUTH_SHELL_NATIVE, oauthShellCookieOptions());
}
...
export function oauthShellSetCookieHeader(cookieDomain) {
  ...
  return `${TORP_OAUTH_SHELL_COOKIE}=${TORP_OAUTH_SHELL_NATIVE}; Path=/; Max-Age=600; Secure; HttpOnly; SameSite=Lax${domainPart}`;
}
...
export function oauthShellClearCookieHeader(cookieDomain) {
  const domain = String(cookieDomain || process.env.WORKOS_COOKIE_DOMAIN || "").trim();
  const domainPart = domain ? `; Domain=${domain}` : "";
  return `${TORP_OAUTH_SHELL_COOKIE}=; Path=/; Max-Age=0; Secure; HttpOnly; SameSite=Lax${domainPart}`;
}

/** Route handler — clear shell marker after successful callback. */
export async function clearOAuthNativeShell() {
  const jar = await cookies();
  jar.set(TORP_OAUTH_SHELL_COOKIE, "", { ...oauthShellCookieOptions(), maxAge: 0 });
}
