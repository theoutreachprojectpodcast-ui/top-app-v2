...
export function mapTrustedResourcesDbRowToTrustedRow(db = {}) {
  ...
  return {
    ...
    catalogId: db.id != null ? String(db.id) : "",
    sort_order: Number(db.sort_order) || 0,
    trustedResourceSlug: String(db.slug || "").trim(),
    ...
    full_description: String(db.full_description || db.long_description || "").trim(),
    long_description: String(db.long_description || db.full_description || "").trim(),
    detail_field_sources: db.detail_field_sources ?? null,
    detail_scraped_at: db.detail_scraped_at ?? null,
    detail_review_status: String(db.detail_review_status || "").trim(),
    ...
    tiktokUrl: db.tiktok_url || "",
    mission: String(db.mission || "").trim(),
    who_they_serve: String(db.who_they_serve || "").trim(),
    services: db.services ?? "",
    service_area: String(db.service_area || "").trim(),
    donation_url: String(db.donation_url || "").trim(),
    volunteer_url: String(db.volunteer_url || "").trim(),
    intake_url: String(db.intake_url || "").trim(),
    events_url: String(db.events_url || "").trim(),
    resource_library_url: String(db.resource_library_url || "").trim(),
    resource_links: db.resource_links ?? null,
    last_reviewed_at: db.last_reviewed_at ?? null,
    source_notes: String(db.source_notes || "").trim(),
    contact_email: String(db.contact_email || "").trim(),
    contact_phone: String(db.contact_phone || "").trim(),
    ...
    raw: {
      profile: {
        ...
        full_description: db.full_description || db.long_description || "",
        city: db.city,
        state: db.state,
