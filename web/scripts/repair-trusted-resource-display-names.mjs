/**
 * Backfill Trusted Resources display names in Supabase catalog (`proven_allies`).
 *
 * Strategy:
...
const catalog = await sb.from("proven_allies").select("id,ein,display_name,website_url").limit(5000);
if (!catalog.error) {
  rows = catalog.data || [];
...
for (const row of rows) {
  ...
  const current = sourceTable === "proven_allies"
  ...
  if (next !== current) {
    updates.push({
      id: sourceTable === "proven_allies" ? row.id : row.ein,
      display_name: next,
    });
  }
}
